...
import { getBeneficiaryById } from '@/lib/services/beneficiary'
import { BeneficiaryForm } from '@/components/dashboard/beneficiaries/BeneficiaryForm'

export const metadata = {
  title: 'Edit Beneficiary — Open Welfare Admin',
...
export default async function EditBeneficiaryPage(
  ...
  const { data: beneficiary, error } = await getBeneficiaryById(params.id)
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-white tracking-tight">Edit Beneficiary</h1>
        <p className="mt-1 text-sm text-zinc-400">Update details for {beneficiary.full_name}.</p>
      </div>

      {/* Form card */}
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6 md:p-8">
        <BeneficiaryForm initialData={beneficiary} />
      </div>
    </div>
  )
}
